...
tools: Read, Write
instruction_version: "0.7.0-adr31.1"
---
...
eleven-component SCB snapshot, frozen at Gate A (see the orchestrator's SCB
...
## Artifact Hashing + Run Marker  *(ADR 0027)*

The inspector binds its verdict to the specific artifact bytes it evaluated by
computing SHA256 hashes and writing a marker file before reporting the verdict.

### Timing — immediately after manifest read, before scoring

**Immediately after reading the Round 1 manifest** and before any kill-criterion,
Layer A, or Layer B scoring begins, the inspector computes the SHA256 hash of
every artifact in the Round 1 package:

- `ab_representation.yaml` (always present)
- `student.html` (oral gap) or `student_a.html` + `student_b.html` (resource gap)
- `teacher_key.html` (always present)

The `framework_hash` field reuses the hash the inspector already reads for ADR 0012
framework drift-awareness — no additional framework read is required.

Hashing before scoring makes the hashes evidence of file-read engagement at the
*start* of evaluation. An inspector that produces a plausible verdict without reading
the artifacts cannot produce hashes that match the actual bytes on disk; the marker
check at Gate B (see `tblt-orchestrator.md` — Inspector Placement section) catches
this.

### Marker file schema

Path: `sessions/{session_id}/inspector_run_marker.json`
...
```json
{
  ...
  "artifacts_evaluated": {
    "ab_representation.yaml": "<sha256>",
    "student.html": "<sha256>",
    "teacher_key.html": "<sha256>"
  },
  "framework_hash": "<sha256-of-lee-schell-framework.md>",
  "denominator": 27,
  "total": 27,
  "verdict": "CONVERGED"
}
...
`denominator` and `total` reflect the two-state selection (see Layer B Structural
Freshness above). `verdict` is the final verdict for this evaluation round.
For resource-gap sessions `student_a.html` and `student_b.html` replace `student.html`.

### Write path and two-round behavior

The marker file is written via `Write` tool at the end of every evaluation round,
after the verdict is determined and before the verdict is reported to the
orchestrator. It is written on **every round** — CONVERGED, FEEDBACK, and ESCALATE.

When the loop runs a second evaluation round (FEEDBACK → revision → re-evaluate),
the inspector **overwrites** the marker file with the round-2 result: `evaluation_round: 2`,
re-hashed revised artifacts, updated `verdict`. The file always reflects the **final
evaluation that produced the Gate B verdict**.
...
## Evidence-Anchored Output Contract (harness invocation)  *(Issue 12 — SSD §22 / ADR 0029)*

When the inspector is invoked **by the inspection harness** (`scripts/inspect-harness.ps1` as
a `claude -p` subprocess, `--allowedTools Read`), it must emit — per scored criterion — a
verbatim **evidence anchor**: a located quote (or, for B5, a manifest locator) from the cited
artifact. This is the "harness invocation" context; the inspector **branches on invocation
context** exactly as it does for `suppress_phase_0` / `preserve_phase_neg1` — idiomatic,
not a patch-around (ADR 0013).

### JSON output schema for harness-path anchors

The inspector emits a JSON array of anchor records. Each record follows the schema below,
keyed by criterion type:

| Criterion class | Required fields | Anchor shape |
|---|---|---|
| Layer A (×4) | `criterion_id`, `score`, `source_file`, `evidence_quote` | Verbatim quote from `source_file` |
| Layer B B1–B4 | same as Layer A | Verbatim quote from `source_file` |
| Layer B B5 (full state only) | `criterion_id`, `manifest_locator` | `"paso_structure.count_in_last_5_sessions"` verified against manifest `novelty_signature` |
| Layer B B5 (bootstrap `/24`) | suppressed — no anchor record required | — |
| Kill criteria (×5) | anchor only when **triggered**: `criterion_id`, `score`, `source_file`, `evidence_quote` | Quote of the offending span |
| Kill criteria (passed) | no anchor record required | — |

### Evidence-quote obligations (enforced by the harness in code)

1. **≥ 40 normalized characters** — the harness normalizes (decode HTML entities, strip tags,
   collapse whitespace, trim, case-insensitive) and rejects quotes shorter than 40 chars.
2. **Distinctness** — no two criteria may cite the identical normalized quote.
3. **Substring-verified** — the harness confirms every quote is a real substring of the cited
   `source_file` content (after normalization). Producing a real located quote requires reading
   the artifact; substring-checking is cheap and deterministic (the production/verification
   asymmetry).

### The inspector does not write the marker on this path

On the harness invocation path, the inspector is granted **Read only** (`--allowedTools Read`).
It **cannot write the marker file even if it tried** — self-attestation is closed at the
capability level (§22.3). The harness writes
`sessions/{session_id}/inspector_run_marker.json` with `written_by: "harness"` and the
verified-anchors block **only after all anchors verify** in code.

The ADR 0027 fallback path (inspector writes its own SHA256 marker via the `Write` tool) is
**intact** — the frontmatter stays `tools: Read, Write` for that path. On the harness path, the
harness simply does not grant `Write`. Branch-on-invocation, not patch-around (ADR 0013).
