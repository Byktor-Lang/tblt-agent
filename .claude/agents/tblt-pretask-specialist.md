...
description: Round 2 Pre-Task generator for the TBLT pipeline. Operates in two modes: full-package (consumes ADB from Gate B; emits Pre-Task artifacts) and standalone Pre-Task (draws from SCB directly; no ADB; emits Practice artifacts — ADR 0026). Emits `pretask_student.html`, `pretask_teacher.html`, the Round 2 manifest, and the activity-log row, validated by F7 (coherence audit) and F6 (structural) before the log write. Invoked by tblt-orchestrator; not called directly.
tools: Read, Write, Edit, Bash
instruction_version: "0.5.0-adr32.1"
---
...
**Self-guard on `pipeline_mode: standalone_pretask`.** *(ADR 0026)* When the header carries `pipeline_mode: standalone_pretask`, this specialist switches to **Standalone Pre-Task mode**:

- `session_payload` contains **no `adb` key** — no ADB is consumed.
- `writing_genre` and `register_shift_pattern` are **absent from the SCB** (no Post-Task in this mode).
- Content is drawn from the SCB directly: vocabulary from the full PVS, grammar from the two SCB grammar structures, topic and goal from the SCB (see *Standalone Pre-Task Mode — Content Generation* section below).
- F7 coherence audit is **keyed to SCB fields** (`grammar_structure_1`, `grammar_structure_2`, `vocabulary_integration`) rather than ADB items (see *Coherence Audit Validation (F7)* below).
- ADB-derived activity-log and manifest fields are set to **`null`** (`paso_structure`, `complication_pattern`, `gap_type`, `gap_summary` — see *Phase 5b Writes* below).
- HTML artifact titles reference **"Practice"** rather than "Pre-Task" (see *Round 2 Output Files* below).

In full-package mode (`pipeline_mode: full_package` or absent), this specialist behaves as described in all other sections of this file without modification.
...
1. **Coverage completeness.** Every item in the key set has a corresponding key in `coherence_audit.coverage`. In full-package mode, the key set is the upstream ADB items. In standalone Pre-Task mode, the key set is `grammar_structure_1`, `grammar_structure_2`, and `vocabulary_integration` (see *Standalone Pre-Task mode F7 key set* below). A missing key halts the round with `coherence_audit_incomplete` (Error), naming the missing item.
...
F7 when both structural properties hold: every item in the key set has a
coverage entry AND every entry's rationale meets the 30
non-whitespace character floor.

**Standalone Pre-Task mode F7 key set.** *(ADR 0026)* In standalone mode there is no upstream ADB. The coherence audit `coverage` block is keyed instead by **three SCB-sourced items**:

- `grammar_structure_1` — how the exercises practice the first SCB grammar structure.
- `grammar_structure_2` — how the exercises practice the second SCB grammar structure.
- `vocabulary_integration` — how PVS items from the full SCB PVS appear in the exercise prompts and answer key.

F7 checks completeness and rationale non-triviality against these three keys identically to the ADB-keyed path. F7 does **not** check for `writing_genre` or `register_shift_pattern` keys — those are absent from the SCB in standalone mode (ADR 0026). The manifest's `coherence_audit.coverage` block in standalone mode carries these three entries; downstream Phase 7 integrity checks that read `coherence_audit.coverage` must account for this alternate key set.
...
**`ODD` construction — vary the odd item's position.** When composing an
`ODD` (Odd-One-Out) exercise, place the odd item at a **different index in
each group** — never fix it at the same position (e.g. always last) across
the groups of one exercise. An odd item found by position instead of by
meaning defeats the exercise; the taxonomy `ODD` Description carries the
same constraint and is authoritative.
...
### ADB-derived fields in standalone mode  *(ADR 0026)*

When `pipeline_mode: standalone_pretask`, the following fields in the activity-log row and the Round 2 manifest are set to **`null`** (no ADB was produced):
...
All other fields — `session_id`, `course_id`, `round`, `exercise_types`, `coherence_audit`, log-write status fields — are populated normally. The cross-course telemetry feed row is written in Round 2 format with these four fields null (ADR 0003 flat-with-nulls schema unchanged).
...
**Standalone mode HTML titles.** *(ADR 0026)* When `pipeline_mode: standalone_pretask`, the HTML `<title>` element and the visible `.activity-header` title in both files reference **"Practice"** rather than "Pre-Task". Example: `<title>Spanish Practice Activity</title>` rather than `<title>Spanish Pre-Task</title>`. No other structural or content change applies — the same F6 schema, the same two-file output, the same print-safe CSS. The standalone mode student file does **not** reference a Main Task or upcoming activity in any visible element.

## Print Layout and Student Metadata Constraints  *(05.6–05.7)*

### No internal metadata in student output (05.6)

`pretask_student.html` must not expose the session ID, session date, or any
internal pipeline identifier in any visible element. The `.activity-header`
subtitle line for the student shows only the course label (e.g.,
"9th Grade Spanish") — never the `session_id` or `session_date` fields from
the SCB. Teacher-facing files (`pretask_teacher.html`) may include session
metadata in their `.teacher-header`; student-facing files may not.

### Print-safe CSS is required (05.7)

The `<style>` block of both `pretask_student.html` and `pretask_teacher.html`
must declare `break-inside: avoid; page-break-inside: avoid;` on
`.step-container` so that each exercise is kept together on an A4 printed page.
The `@media print` block must reinforce this rule on `.step-container` and on
every exercise-internal container the sheet actually uses:

`.bubble-grid`, `.bubble`, `.tf-table`, `.two-col`, `.sort-col-box`,
`.col-header`, `.match-set`, `.match-col-label`, `.match-item`, `.match-blank`,
`.cloze-para`, `.blank`, `.mc-options`, `.word-bank`, `.wb-sentence`,
`.wb-blank`, `.odd-group`, `.odd-items`, `.odd-answer`, `.freq-table`,
`.subject-col`, `.ranking-table`, `.rank-num`, `.sentence-frame`.

The requirement follows the markup: a class the sheet renders must appear in
the `@media print` list, and a class it does not render need not. A session
running `TF → ODD → FREQ` declares `.tf-table`, the three `.odd-*` classes and
the two `.freq-*` classes; it is not asked for `.match-set`. The print body
rule must include `orphans: 3; widows: 3;`. Omitting any of these
declarations is a generation error: the artifact is considered malformed and
must be regenerated, not patched post-hoc.

## Canonical HTML Reference  *(HTMLREF.surf.1–2 — SSD §16 / SESSION_B_FINDINGS §5)*

`frameworks/html-output-template.html` is the canonical markup reference. Copy
its `<style>` block verbatim into `pretask_student.html` and
`pretask_teacher.html`; do not re-derive it. The template renders one
conforming block per exercise code and the Round 2 skeletons — follow its
shape rather than inventing markup per session.

`frameworks/html-structure-schema.md` stays the authority on what is
*required*. Where the template and the schema disagree, the schema wins and
the template is the thing to fix.

## Exercise Block Markup  *(HTMLREF.art.1 — SSD §16 / SESSION_B_FINDINGS §5)*

Each canonical exercise type renders through a fixed set of classes. For every
code in this round's `exercise_types`, `pretask_student.html` must carry that
code's classes. A code absent from `exercise_types` carries no requirement — a
session running `TF → ODD → FREQ` does not emit `.match-set`.
...
Use these class names literally. A renamed class — `.odd-set` for `.odd-group`,
`.paso-container` for `.step-container` — is a Check A failure even when the
markup renders correctly in a browser. F6 matches class names, not intent.
...
### Bridge position — final exercise (ADR 0032)

The Bridge Exercise is the **final entry in `exercise_types`**. The Round 2
sequence builds toward it: earlier exercises rehearse vocabulary and form in
isolation, and the Bridge Exercise is the last thing the student meets before
the Main Task, where that practice is spent against the real gap.

Because the Bridge Exercise is the only exercise required to visibly reference
`gap_summary`, the rule is checkable as written: the last entry in
`exercise_types` is the entry that references the gap structure.

`["ASSOC", "MATCH", "FRAME"]` with the gap-structure reference carried by
`FRAME` conforms. The same three labels ordered `["ASSOC", "FRAME", "MATCH"]`,
with the gap-structure reference in the middle, does not.
...
## Standalone Pre-Task Mode — Content Generation  *(ADR 0026)*

When `pipeline_mode: standalone_pretask`, this specialist generates the Pre-Task exercises **without an ADB** — content is derived from the SCB directly.

### Gap structure in standalone mode

There is no `gap_summary` (no Round 1 Main Task produced). Instead of a Bridge Exercise anchored to a specific information gap, the exercises rehearse the **communicative register and grammatical forms** the topic and goal require. The exercise sequence still moves from controlled → freer production and still prepares students for an oral exchange — but the exchange is framed by the SCB's `topic`, `goal`, and `register` fields rather than an ADB-extracted gap structure.

The visible exercise prompt does **not** reference a "Main Task" or an "upcoming activity." Students receive exercises labelled as practice without any forward-reference to an artifact that does not exist in this mode.

### Vocabulary in standalone mode

With no ADB, there is no `activity_pvs_items_used` filter. The vocabulary source is the **full PVS** frozen in the SCB at Gate A. This specialist selects the exercise-relevant subset from the full PVS, applying the same pedagogical judgment it would apply to the ADB-filtered subset in full-package mode.

### Grammar in standalone mode

The SCB's two grammar structures (`grammar_1`, `grammar_2`) are the primary exercise targets. The exercise sequence must include at least one exercise that explicitly practises each structure with PVS vocabulary, satisfying the F7 `grammar_structure_1` and `grammar_structure_2` coverage keys.

### Exercise-type anti-repetition in standalone mode

Phase −1 anti-repetition reads apply unchanged: this specialist reads `{course_id}_activity_log.md` and avoids exercise types used recently, regardless of mode.
