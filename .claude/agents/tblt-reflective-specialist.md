...
instruction_version: "0.3.0-htmlref.art.3"
---
...
its `<style>` block verbatim into `posttask.html` and `posttask_teacher.html`;
do not re-derive it. The template renders the Round 3 skeleton — including the
`.writing-task-container` nesting below — so follow its shape rather than
inventing markup per session.
...
<div class="writing-task-container" style="break-inside: avoid;">
  ...
  <section class="checklist-section">
    <ul class="single-column-list">
      <li class="list-item">[check 1]</li>
      <li class="list-item">[check 2]</li>
      <li class="list-item">[check 3]</li>
    </ul>
  </section>
</div>
```
...
not nested inside each other. The `.checklist-section` carries one
`.single-column-list` holding **at least three** `.list-item` entries; three is
the floor, not the target — a checklist with five items is fine, one with two
fails. F6 structural validation verifies this
