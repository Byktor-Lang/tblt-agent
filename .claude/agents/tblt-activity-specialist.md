...
instruction_version: "0.6.0-htmlref.art.2"
---
...
its `<style>` block verbatim into every Round 1 artifact (`student.html` or
`student_a.html` + `student_b.html`, plus `teacher_key.html`); do not
re-derive it. The template renders the Round 1 and teacher-key skeletons —
follow its shape rather than inventing markup per session.
...
## Round 1 Step Markup  *(HTMLREF.art.2 — SSD §16 / SESSION_B_FINDINGS §5)*

Every Paso in a Round 1 student artifact is wrapped in `.step-container`, with
its heading in `.step-header`:

```html
<div class="step-container">
  <h2 class="step-header">Paso 1 — [step title]</h2>
  <p>[step instructions]</p>
</div>
```

Use these two class names **literally**. F6 matches class names verbatim, so a
Spanish-language rename — `.paso-container` for `.step-container` — is a Check A
failure even though the page renders identically in a browser. Translating the
*visible text* to Spanish is correct and expected; translating the *class name*
is not.
...
**All six fields must be present in the emitted YAML block in both
states.** In bootstrap state, `label` and `sessions_since_last_use` are
written as the YAML literal `null` — they are **not omitted** from the
block. The bootstrap form is:

```yaml
novelty_signature:
  paso_structure:
    label: null
    sessions_since_last_use: null
    count_in_last_5_sessions: 0
  complication_pattern:
    label: null
    sessions_since_last_use: null
    count_in_last_5_sessions: 0
...
The bootstrap trigger is the **total session count in the activity
log** — not the `count_in_last_5_sessions` value. A first-use paso
structure (one that has never appeared in the log before) still receives
the bootstrap `null` labels when the log holds fewer than 5 sessions;
`count_in_last_5_sessions: 0` for a first-use structure does **not**
override the bootstrap rule.
